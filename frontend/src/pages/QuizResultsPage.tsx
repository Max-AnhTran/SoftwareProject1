...
const QuizResultsPage: React.FC = () => {
    ...
    const fetchResults = async () => {
        try {
            const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/quizzes/${id}/results`);
    ...
    const totalAttempts = totalQuestions > 0 ? Math.round(totalAnswers / totalQuestions) : 0;
    ...
    return (
        <div className="space-y-6 p-6 max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-2 gap-6">
                ...
                <StatCard label="Correct Answer Percentage" value={`${correctPercentage.toFixed(1)}%`} />
            ...
            <ul className="space-y-4">
                {r.perQuestionStats.map((q) => (
                    <li key={q.questionId} className="bg-white p-5 rounded-2xl shadow flex justify-between">
